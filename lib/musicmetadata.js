...
MusicMetadata.prototype.parse = function () {
  ...
  this.stream.once('data', function (result) {
    ...
    var parser = module(self.stream, self.readEvents.bind(self));
...
MusicMetadata.prototype.readEvents = function (event, value) {
  ...
  //if the event has been this.aliased then we need to clean it before
  ...
  if (alias) {
    ...
    //if we haven't previously this.aliased this tag then
