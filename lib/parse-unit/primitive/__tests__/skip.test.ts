...
test("unit: read unit and skip a specified length", async () => {
  const tokenizer = new BufferTokenizer(buffer);

  await expect(readUnitFromTokenizer(tokenizer, pad(u8, 3))).resolves.toBe(0x00);
  await expect(readUnitFromTokenizer(tokenizer, bytes(3))).resolves.toEqual(Uint8Array.of(0x80, 0xff, 0x81));
});
