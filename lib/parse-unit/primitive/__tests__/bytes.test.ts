...
import { bytes, bytesTokenizer } from "../bytes";
...
test("unit: buffer tokenizer", async () => {
  const tokenizer = new BufferTokenizer(buffer);

  await expect(readUnitFromTokenizer(tokenizer, bytesTokenizer(3))).resolves.toEqual(
    new BufferTokenizer(Uint8Array.of(0x00, 0x01, 0x7f))
  );
  await expect(readUnitFromTokenizer(tokenizer, bytesTokenizer(3))).resolves.toEqual(
    new BufferTokenizer(Uint8Array.of(0x80, 0xff, 0x81))
  );
  await expect(readUnitFromTokenizer(tokenizer, bytesTokenizer(3))).resolves.toEqual(
    new BufferTokenizer(Uint8Array.of(0xe6, 0x47, 0x80))
  );
  await expect(readUnitFromTokenizer(tokenizer, bytesTokenizer(3))).resolves.toEqual(
    new BufferTokenizer(Uint8Array.of(0x6a, 0x5e, 0x4b))
  );
  await expect(readUnitFromTokenizer(tokenizer, bytesTokenizer(3))).resolves.toEqual(
    new BufferTokenizer(Uint8Array.of(0x2e, 0x63, 0x5a))
  );
  await expect(readUnitFromTokenizer(tokenizer, bytesTokenizer(3))).resolves.toEqual(
    new BufferTokenizer(Uint8Array.of(0x6d, 0xdd, 0x1d))
  );
});
