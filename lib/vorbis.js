...
var Vorbis = module.exports = function(stream) {
    this.stream = stream;
    this.parse();
};
    ...
    try {
      ...
      if (cb.position === 'comment') {
        ...
        if (split[0] === 'METADATA_BLOCK_PICTURE') {  
          ...
          picture.type = common.PICTURE_TYPE[strtok.UINT32_BE.get(decoded, 0)];
          
          var mimeLen = strtok.UINT32_BE.get(decoded, offset += 4);
          picture.format = decoded.toString('utf-8', offset += 4, offset + mimeLen);

          var descLen = strtok.UINT32_BE.get(decoded, offset += mimeLen);
          picture.description = decoded.toString('utf-8', offset += 4, offset + descLen);
        
          picture.width = strtok.UINT32_BE.get(decoded, offset += descLen);
          picture.height = strtok.UINT32_BE.get(decoded, offset += 4);
          picture.colour_depth = strtok.UINT32_BE.get(decoded, offset += 4);
          picture.indexed_color = strtok.UINT32_BE.get(decoded, offset += 4);
   
          var picDataLen = strtok.UINT32_BE.get(decoded, offset += 4);
          picture.data = decoded.slice(offset += 4, offset + picDataLen);
