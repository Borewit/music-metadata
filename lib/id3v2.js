...
id3v2.prototype.parse = function(){
	...
	strtok.parse(self.stream, function(v,cb){
		if (v === undefined) return new strtok.StringType(3, 'ascii');
        ...
        if (typeof v === 'object' && v.length === cb.frameLength){
			switch (cb.major){
				case 2 :
                    var encoding = getTextEncoding(v[0]);
					var frame = parser.readData(v.slice(1,v.length), cb.frameId, encoding , null, cb.major);
                    self.emit(cb.frameId, frame);
					return new strtok.StringType(3, 'ascii');
				case 3 : case 4:
                    var sliced = v.slice(0,2);
                    var frameFlags = readFrameFlags(v.slice(0,2));
                    var encoding = getTextEncoding(v[2]);
					var frame = parser.readData(v.slice(3,v.length), cb.frameId, encoding, frameFlags, cb.major);
                    self.emit(cb.frameId, frame);
					return new strtok.StringType(4, 'ascii');
			}
        }
		...
		if (typeof v === 'string'){
            ...
            // Last frame
            if (v === '' || v === '\u0000\u0000\u0000\u0000') return strtok.DONE;
            
            //check first char is a letter             
            if('ABCDEFGHIJKLMNOPQRSTUVWXYZ'.search(v[0]) === -1) return strtok.DONE;
	...
	function readTagSize(buffer, offset) {
	  offset || (offset = 6);
