...
exports.readData = function readData (b, type, flags, major) {
  ...
  switch (type[0] === 'T' ? 'T*' : type) {
    case 'T*': // 4.2.1. Text information frames - details
      var text = decodeString(b.slice(1), encoding).replace(/\x00+$/, '')
      // id3v2.4 defines that multiple T* values are separated by 0x00
      if (type === 'TXXX') {
        output = text.split(/\x00/)
        output = {description: output[0], text: output[1]}
        // Strip of BOM field
        output.text = output.text.length > 0 && output.text[0] === '\uFEFF' ?
          output.text.substr(1) : output.text
        output = [output]
      } else {
