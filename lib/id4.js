...
id4.prototype.parse = function() {
    ...
    strtok.parse(self.stream, function(v,cb){
        ...
        if(total >= cb.atomContainerLength - 8){
            self.emit('done');
            return strtok.DONE;
        }
        ...
        if (typeof v === 'object' && cb.atomContainer ==='ilst'){
            ...
            //ugly hack to get the system to process the © symbol
            //not investigated fully but node is not converting the binary code properly to
            //utf-8 (as of node v0.3.1)
            function setCharAt(str,index,chr) {
                if(index > str.length-1) return str;
                return str.substr(0,index) + chr + str.substr(index+1);
            }
            
            if(cb.atomName.charCodeAt(0) === 65533){
                cb.atomName = setCharAt(cb.atomName,0,'©');
            }
        ...
        if (!cb.atomLength || cb.atomLength === -1){
            cb.atomLength = v;
            return new strtok.StringType(4, 'utf8');
        }
