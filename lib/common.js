...
exports.detectMediaType = function (header) {
  ...
  var tag = 'id3v1';
  if (bufferEqual(asfGuidBuf, header.slice(0, asfGuidBuf.length))) {
    tag = 'asf';
  } else if ('ID3' === header.slice(0, 3).toString('ascii')) {
    tag = 'id3v2';
  } else if ('ftypM4A' === header.slice(4, 11).toString('ascii')) {
    tag = 'id4';
  } else if ('ftypmp42' === header.slice(4, 12).toString('ascii')) {
    tag = 'id4'
  } else if ('OggS' === header.slice(0, 4).toString('ascii')) {
    tag = 'ogg';
  } else if ('fLaC' === header.slice(0, 4).toString('ascii')) {
    tag = 'flac';
  } else if ('MAC' === header.slice(0, 3).toString('ascii')) {
    tag = 'monkeysaudio';
  }
  return tag;
}
