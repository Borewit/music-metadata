  ...
  strtok.parse(stream, function (v, cb) {
    ...
    switch (cb.state) {
      case 0:
        ...
        var lenExp = v.descriptorBytes - ApeDescriptor.len
        if (lenExp > 0) {
          cb.state = 1
          return new strtok.IgnoreType(lenExp) // Ignore extension header
        }
        // could fall through, but standard doesn't allow it
        cb.state = 2
        return ApeHeader
