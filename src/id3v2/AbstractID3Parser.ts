    ...
    return tokenizer.peekToken(ID3v2Token.Header)
      ...
      .then((isID3) => {
        if (isID3) {
          ...
          return id3parser.parse(tokenizer, options);
        } else {
          return {}; // Empty native metadata
        }
      })
      .then((id3v2Metadata: INativeAudioMetadata) => {
        // merge ID3v2 metadata with whatever came after the ID3v2 header
        return this._parse(tokenizer, options).then((metadata) => {
          for (const tagType in id3v2Metadata) {
            metadata.native[tagType] = id3v2Metadata[tagType];
          }
          return metadata;
        });
      })
