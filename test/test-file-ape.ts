...
describe("Parse APE (Monkey's Audio)", () => {
  test.each(Parsers)("parser: %s", async (parser) => {
    ...
    expect(
      ...
      "metadata.native.APEv2 should be defined"
    ).toBeDefined();
    ...
    expect(common.picture[0].data.length, "common.picture 0 length").toBe(
      48_658
    );
    expect(common.picture[1].format, "common.picture 1 format").toBe(
      "image/jpeg"
    );
    expect(common.picture[1].data.length, "common.picture 1 length").toBe(
      48_658
    );
    ...
    expect(
      native["Cover Art (Front)"][0].data.length,
      "raw cover art (front) length"
    ).toBe(48_658);
    expect(
      native["Cover Art (Back)"][0].data.length,
      "raw cover art (front) length"
    ).toBe(48_658);
  });
});
...
describe("Parse APEv2 header", () => {
  test("Handle APEv2 with item count to high(issue #331)", async () => {
    ...
    expect(common.title, "common.title").toBe(
      "Push The Feeling On (Groove Phenomenon Remix)"
    );
