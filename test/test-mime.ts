...
// ToDo: ass ".ac3"
const audioExtension = [".aac", ".mp3", ".ogg", ".wav", ".flac", ".m4a"] as const;
...
function handleError(extension: typeof audioExtension[number], err: Error) {
  switch (extension) {
    ...
    case ".wav":
    case ".ogg":
      expect(err.message.startsWith("FourCC"), `Only FourCC error allowed, got: ${err.message}`).toBe(true);
      break;
...
test.each(audioExtension)("should map on extension as well %s", async (extension) => {
  const streamReader = new SourceStream(buf);
  try {
...
describe("Resolve MIME based on content", () => {
  test("should fall back on content detection in case the extension is useless", async () => {
    const filePath = join(samplePath, "mp3", "1a643e9e0743dee8732554d0e870055a");
  ...
  test("should throw error on unrecognized MIME-type", async () => {
    const streamReader = new SourceStream(buf);
    const rejected = expect(() => parseStream(streamReader, { mimeType: "audio/not-existing" })).rejects;
    await rejected.toBeInstanceOf(Error);
    await rejected.toHaveProperty("message", "Failed to determine audio format");
  });
  ...
  test("should throw error on recognized MIME-type which is not supported", async () => {
    ...
    const rejected = expect(() => parseStream(stream, { mimeType: "audio/not-existing" })).rejects;
    await rejected.toBeInstanceOf(Error);
    await rejected.toHaveProperty("message", "Guessed MIME-type not supported: image/jpeg");
  });
  ...
  const fileTypes: [string, string, string][] = [
    ...
    ["MPEG-4 / m4a", "MusicBrainz - Beth Hart - Sinner's Prayer.m4a", "M4A/mp42/isom"],
    ["MPEG-4 / m4b", join("mp4", "issue-127.m4b"), "M4A/3gp5/isom"],
    ["MPEG-4 / mp4", join("mp4", "Mr. Pickles S02E07 My Dear Boy.mp4"), "mp42/isom"],
    ["FLAC", "flac.flac", "FLAC"],
    ...
    ["WavPack", join("wavpack", "MusicBrainz - Beth Hart - Sinner's Prayer.wv"), "WavPack"],
    ["SV7", join("mpc", "apev2.sv7.mpc"), "Musepack, SV7"],
    ["SV8", join("mpc", "bach-goldberg-variatians-05.sv8.mpc"), "Musepack, SV8"],
    ["DSF", join("dsf", "2L-110_stereo-5644k-1b_04_0.1-sec.dsf"), "DSF"],
    ["MKA", join("matroska", "02 - Poxfil - Solid Ground (5 sec).mka"), "EBML/matroska"],
    ["WebM", join("matroska", "02 - Poxfil - Solid Ground (5 sec).opus.webm"), "EBML/webm"],
  ];
