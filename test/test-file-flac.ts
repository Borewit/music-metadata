...
describe("Parse FLAC", () => {
  ...
  function checkNative(vorbis) {
    ...
    t.strictEqual(
      pic.format,
      "image/jpeg",
      "raw METADATA_BLOCK_PICTURE format"
    );
    t.strictEqual(
      pic.description,
      "",
      "raw METADATA_BLOCK_PICTURE description"
    );
  ...
  it("Support Vorbis METADATA_BLOCK_PICTURE tags", async () => {
    ...
    assert.deepEqual(
      common.picture[0].format,
      "image/jpeg",
      "ommon.picture[0].format"
    );
    assert.deepEqual(
      common.picture[0].data.length,
      107402,
      "ommon.picture[0].data.length"
    );
    assert.deepEqual(
      common.picture[1].format,
      "image/jpeg",
      "ommon.picture[1].format"
    );
    assert.deepEqual(
      common.picture[1].data.length,
      215889,
      "ommon.picture[1].data.length"
    );
  });
