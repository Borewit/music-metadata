...
describe("Mapping definitions", () => {
  ...
  test.each(discogs_tags)(
    "should map Discogs/ID3v2.3/ID3v2.4 tag: %s",
    (tag) => {
...
describe("Track mapping: Beth Hart - Sinner's Prayer", () => {
  test("ID3v2.3/ID3v2.4", async () => {
    ...
    expect(metadata.common.artist, "common.artist").toBe(
    ...
    expect(
      ...
      "id3v23/TXXX:DISCOGS_RELEASE_ID"
    ).toStrictEqual(["4204665"]);
    expect(
      metadata.common.discogs_release_id,
      "id3v23/TXXX:DISCOGS_RELEASE_ID => common.discogs_release_id"
    ).toBe(4_204_665);

    // Check Discogs-tag: DISCOGS_ARTIST_ID
    expect(
      id3v23["TXXX:DISCOGS_ARTIST_ID"],
      "id3v23/TXXX:DISCOGS_ARTIST_ID"
    ).toStrictEqual(["389157", "900313"]);
    expect(
    ...
    expect(
      id3v23["TXXX:DISCOGS_ALBUM_ARTISTS"],
      "id3v23/TXXX:DISCOGS_ALBUM_ARTISTS"
    ).toStrictEqual(["Beth Hart", "Joe Bonamassa"]);
    expect(
      metadata.common.artists,
      "id3v23/TXXX:DISCOGS_ALBUM_ARTISTS => common.discogs_release_id"
    ).toStrictEqual(["Beth Hart", "Joe Bonamassa"]);

    // Check Discogs-tag: DISCOGS_VOTES
    expect(
      id3v23["TXXX:DISCOGS_VOTES"],
      "id3v23/TXXX:DISCOGS_VOTES"
    ).toStrictEqual(["9"]);
    expect(
      metadata.common.discogs_votes,
      "id3v23/TXXX:DISCOGS_VOTES => common.discogs_release_id"
    ).toStrictEqual(9);

    // Check Discogs-tag: STYLE
    expect(id3v23["TXXX:STYLE"], "id3v23/TXXX:STYLE").toStrictEqual([
      "Blues Rock",
    ]);
    expect(
      metadata.common.genre,
      "id3v23/TXXX:STYLE => common.genre"
    ).toStrictEqual(["Rock;Blues", "Blues Rock"]);
    expect(id3v23.TCON, `id3v23/TCON`).toStrictEqual(["Rock;Blues"]); // ToDo: why different in Vorbis

    // Check discogs, CATALOGID mapping
    expect(id3v23["TXXX:CATALOGID"], "id3v23/TXXX:CATALOGID").toStrictEqual([
      "PRAR931391",
    ]);
    expect(
      metadata.common.catalognumber,
  ...
  test("Vorbis/FLAC", async () => {
    ...
    expect(
      metadata.common.discogs_release_id,
      "vorbis/DISCOGS_RELEASE_ID => common.discogs_release_id"
    ).toBe(4_204_665);

    // Check Discogs-tag: DISCOGS_ARTIST_ID
    expect(vorbis.DISCOGS_ARTIST_ID, "vorbis/DISCOGS_ARTIST_ID").toStrictEqual([
      "389157",
      "900313",
    ]);
    expect(
      metadata.common.discogs_artist_id,
      "vorbis/DISCOGS_ARTIST_ID => common.discogs_release_id"
    ).toStrictEqual([389_157, 900_313]);

    // Check Discogs-tag: DISCOGS_ALBUM_ARTISTS
    expect(
      vorbis.DISCOGS_ALBUM_ARTISTS,
      "vorbis/DISCOGS_ALBUM_ARTISTS"
    ).toStrictEqual(["Beth Hart", "Joe Bonamassa"]);
    expect(
      metadata.common.artists,
      "vorbis/DISCOGS_ALBUM_ARTISTS => common.discogs_release_id"
    ).toStrictEqual(["Beth Hart", "Joe Bonamassa"]);
    ...
    expect(
      metadata.common.discogs_votes,
      "vorbis/DISCOGS_VOTES => common.discogs_release_id"
    ).toStrictEqual(9);

    // Check Discogs-tag: STYLE
    expect(vorbis.STYLE, "vorbis/STYLE").toStrictEqual(["Blues Rock"]);
    expect(metadata.common.genre, `vorbis/STYLE => common.genre`).toStrictEqual(
...
describe("Track mapping: Yasmin Levy - Mi Korasón.flac'", () => {
  test("Vorbis/FLAC tags", async () => {
    ...
    // Each Discogs tag should be mapped
    for (const tag of Object.keys(native).filter((t) =>
      t.includes("DISCOGS_")
    )) {
      expect(discogs_tags.includes(tag), `Discog/Vorbis tag: ${tag}`).toBe(
        true
      );
    }
    ...
    expect(
      native.DISCOGS_RELEASE_ID,
      "vorbis/DISCOGS_RELEASE_ID"
    ).toStrictEqual(["3520814"]);
    expect(
      metadata.common.discogs_release_id,
      "vorbis/DISCOGS_RELEASE_ID => common.discogs_release_id"
    ).toBe(3_520_814);
    ...
    expect(
      native.DISCOGS_MASTER_RELEASE_ID,
      "vorbis/DISCOGS_MASTER_RELEASE_ID"
    ).toStrictEqual(["461710"]);
    expect(
    ...
    // Check Discogs-tag: DISCOGS_ARTIST_ID
    expect(native.DISCOGS_ARTIST_ID, "vorbis/DISCOGS_ARTIST_ID").toStrictEqual([
      "467650",
    ]);
    expect(
      metadata.common.discogs_artist_id,
      "vorbis/DISCOGS_ARTIST_ID => common.discogs_artist_id"
    ).toStrictEqual([467_650]);

    // Check Discogs-tag: DISCOGS_ARTIST_NAME ToDo: test multiple artists
    expect(
      native.DISCOGS_ARTIST_NAME,
      "vorbis/DISCOGS_ARTIST_NAME"
    ).toStrictEqual(["Yasmin Levy"]);
    expect(
      metadata.common.artists,
      "vorbis/DISCOGS_ARTIST_NAME => common.artists"
    ).toStrictEqual(["Yasmin Levy"]); // ToDo? remove duplicates
    ...
    expect(
      ...
      "vorbis/DISCOGS_DATE => common.originaldate"
    ).not.toBe("2009"); // Why is not "2009"

    // Check Discogs: DISCOGS_CATALOG
    expect(native.DISCOGS_CATALOG, "vorbis/DISCOGS_CATALOG").toStrictEqual([
      "450010",
    ]);
    expect(
      metadata.common.catalognumber,
      "vorbis/DISCOGS_CATALOG => common.catalognumber"
    ).toStrictEqual(["450010"]);
  });
});
