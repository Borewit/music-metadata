...
test("mp3-id3v24", async () => {
  ...
  expect(common.category, "metadata.common.category").toStrictEqual([
    "Podcast Category",
  ]);
  expect(common.podcastId, "metadata.common.podcastId").toBe("1234");
  expect(common.keywords, "metadata.common.keywords").toStrictEqual([
    "Podcast Keywords",
  ]);
  expect(common.podcasturl, "metadata.common.podcasturl").toBe(
    "http://podcast.url"
  );
  expect(common.subtitle, "metadata.common.subscription").toStrictEqual([
    "Short Description",
  ]);
  expect(common.description, "metadata.common.description").toStrictEqual([
    "Long Description",
  ]);
  expect(common.albumartistsort, "metadata.common.albumartistsort").toBe(
    "Album Artist Sort"
  );
  ...
  expect(common.composersort, "metadata.common.composersort").toBe(
    "Composer Sort"
  );
  ...
  expect(common.comment, "metadata.common.comment").toStrictEqual([
  ...
  expect(common.originalalbum, "metadata.common.originalalbum").toBe(
    "Original Album"
  );
});
...
test("mp4", async () => {
  ...
  expect(common.description, "metadata.common.subscription").toStrictEqual([
    "Short Description",
  ]);
  expect(common.longDescription, "metadata.common.description").toBe(
    "Long Description"
  );
  expect(common.albumartistsort, "metadata.common.albumartistsort").toBe(
    "Album Artist Sort"
