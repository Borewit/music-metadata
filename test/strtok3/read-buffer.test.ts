...
describe.each(tokenizerCases)("tokenizer from %s read options", (_name, load) => {
  ...
  test("default length", async () => {
    ...
    const rst = await getTokenizerWithData("increment-6", load);
    expect(await rst.readBuffer(buf, { offset: 1 }), "default length = buffer.length - option.offset").toBe(5);
  });
  ...
  test("option.maybeLess = true", async () => {
    ...
    expect(len, "should return 3 because no more bytes are available").toBe(3);
  });
...
describe.each(tokenizerCases)("tokenizer from %s peek options", (_name, load) => {
  test("option.offset", async () => {
    const buf = Buffer.alloc(7);
    const rst = await getTokenizerWithData("increment-6", load);
  ...
  test("default length", async () => {
    const buf = Buffer.alloc(6);
    const rst = await getTokenizerWithData("increment-6", load);
    expect(await rst.peekBuffer(buf, { offset: 1 }), "default length = buffer.length - option.offset").toBe(5);
  });
