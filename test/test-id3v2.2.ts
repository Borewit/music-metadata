...
it("should decode id3v2.2", () => {
  ...
  return mm.parseFile(filePath, {duration: true, native: true}).then(result => {
    ...
    t.strictEqual(result.common.gapless, false, 'common.gapless');
    t.isUndefined(result.common.comment, 'common.comment');

    t.isDefined(result.native['ID3v2.2'], 'Native id3v2.2 tags should be present');
    const id3v22 = mm.orderTags(result.native['ID3v2.2']);
