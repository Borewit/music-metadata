...
it("should MusicBrainz tags with id3v2.4", async () => {
  ...
  assert.deepEqual(
    native[i++],
    { id: "TPE1", value: "Explosions in the Sky" },
    "['ID3v2.4'].TPE1"
  );
  assert.deepEqual(
    native[i++],
  ...
  assert.deepEqual(
    native[i++],
    { id: "TDRC", value: "2004-10-12" },
    "['ID3v2.4'].TDRC"
  );
  assert.deepEqual(
    native[i++],
    { id: "TCON", value: "Soundtrack" },
    "['ID3v2.4'].TCON #1"
  );
  assert.deepEqual(
    native[i++],
...

  {
    ...
    assert.deepEqual(
      picTag.value.format,
      "image/jpeg",
      "['ID3v2.4'].APIC #1 format"
    );
    assert.deepEqual(
      picTag.value.type,
      "Cover (front)",
      "['ID3v2.4'].APIC #1 tagTypes"
    );
    assert.deepEqual(
      picTag.value.description,
      "",
      "['ID3v2.4'].APIC #1 description"
    );
    assert.deepEqual(
  ...
  assert.deepEqual(
    native[i++],
    { id: "TCOM", value: "Explosions in the Sky" },
    "['ID3v2.4'].TCOM"
  );
  assert.deepEqual(
    native[i++],
    { id: "TDOR", value: "2004-10-12" },
    "['ID3v2.4'].TDOR"
  );
  assert.deepEqual(
    native[i++],
  ...
  assert.deepEqual(
    native[i++],
    { id: "TPE2", value: "Explosions in the Sky" },
    "['ID3v2.4'].TPE2"
  );
  assert.deepEqual(
    native[i++],
    { id: "TPUB", value: "Hip-O Records" },
    "['ID3v2.4'].TPUB"
  );
  assert.deepEqual(
    native[i++],
    { id: "TSO2", value: "Explosions in the Sky" },
    "['ID3v2.4'].TSO2"
  );
  assert.deepEqual(
    native[i++],
    { id: "TSOP", value: "Explosions in the Sky" },
    "['ID3v2.4'].TSOP"
  );
  assert.deepEqual(
    native[i++],
    { id: "TSRC", value: "USUG10400421" },
    "['ID3v2.4'].TSRC"
  );
  assert.deepEqual(
    native[i++],
    { id: "TXXX:ASIN", value: "B000649YAM" },
    "['ID3v2.4'].TXXX:ASIN"
  );
  assert.deepEqual(
    native[i++],
