...
test("should MusicBrainz tags with id3v2.4", async () => {
  ...
  expect(common.musicbrainz_artistid, "MusicBrainz Artist Id").toStrictEqual([
    "4236acde-2ce2-441c-a3d4-38d55f1b5474",
  ]);
  expect(common.musicbrainz_recordingid, "MusicBrainz Recording Id").toBe(
    "84851150-a196-48fa-ada5-1a012b1cd9ed"
  );
  expect(
    common.musicbrainz_albumartistid,
    "MusicBrainz Release Artist Id"
  ).toStrictEqual(["4236acde-2ce2-441c-a3d4-38d55f1b5474"]);
  expect(
    common.musicbrainz_releasegroupid,
    "MusicBrainz Release Group Id"
  ).toBe("afe7c5d8-f8bc-32cf-b77d-8fb8561989a7");
  expect(common.musicbrainz_albumid, "MusicBrainz Release Id").toBe(
    "2644f258-0619-4beb-a150-0c0069ca6699"
  );
  expect(common.musicbrainz_trackid, "MusicBrainz Track Id").toBe(
    "d87d56d0-9bd3-3199-8ff3-d03dff3abb13"
  );
  ...
  const nativeExpected: [number, { id: string; value: any }, string][] = [
    ...
    [
      21,
      { id: "TXXX:Artists", value: "Explosions in the Sky" },
      "['ID3v2.4'].TXXX:Artists",
    ],
    [
      22,
      { id: "TXXX:BARCODE", value: "602498644102" },
      "['ID3v2.4'].TXXX:BARCODE",
    ],
    [
      23,
      { id: "TXXX:CATALOGNUMBER", value: "B0003663-02" },
    ...
    [
      33,
      { id: "TXXX:PERFORMER", value: "Explosions In The Sky" },
      "['ID3v2.4'].PERFORMER",
    ],
    [34, { id: "TXXX:SCRIPT", value: "Latn" }, "['ID3v2.4'].'SCRIPT"],
    [
      35,
      { id: "TXXX:originalyear", value: "2004" },
      "['ID3v2.4'].'originalyear",
    ],
    [
      36,
      {
        ...
        value: {
          identifier: Buffer.from([
            0x38, 0x34, 0x38, 0x35, 0x31, 0x31, 0x35, 0x30, 0x2d, 0x61, 0x31,
            0x39, 0x36, 0x2d, 0x34, 0x38, 0x66, 0x61, 0x2d, 0x61, 0x64, 0x61,
            0x35, 0x2d, 0x31, 0x61, 0x30, 0x31, 0x32, 0x62, 0x31, 0x63, 0x64,
            0x39, 0x65, 0x64,
          ]),
...

  {
    ...
    expect(picTag.value.format, "['ID3v2.4'].APIC #1 format").toBe(
      "image/jpeg"
    );
    expect(picTag.value.type, "['ID3v2.4'].APIC #1 tagTypes").toBe(
      "Cover (front)"
    );
