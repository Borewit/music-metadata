...
function check_Nirvana_In_Bloom_commonTags(common: ICommonTagsResult) {
  ...
  expect(common.barcode, "common.barcode (including leading zero)").toBe(
    "0720642442524"
  );
...
function check_Nirvana_In_Bloom_VorbisTags(vorbis: INativeTagDict) {
  ...
  expect(cover.format, "vorbis.METADATA_BLOCK_PICTURE format").toBe(
    "image/jpeg"
  );
  expect(cover.type, "vorbis.METADATA_BLOCK_PICTURE tagTypes").toBe(
    "Cover (front)"
  );
  // test exact contents too
  expect(cover.data.length, "vorbis.METADATA_BLOCK_PICTURE length").toBe(
    30_966
  );
...
describe("Parsing Ogg/Opus", () => {
  ...
  describe("decode: Nirvana - In Bloom - 2-sec.opus", () => {
    ...
    test.each(Parsers)("%j", async (parser) => {
      ...
      expect(format.tagTypes, "format.tagTypes").toStrictEqual(["vorbis"]);
      expect(format.numberOfSamples, "format.numberOfSamples = 96000").toBe(
        96_000
      );
...
describe("Parsing Ogg/Speex", () => {
  ...
  test("check for ogg-multipage-metadata-bug", async () => {
    ...
    expect(result.common.title, "title").toBe(
      "Modestep - To The Stars (Break the Noize & The Autobots Remix)"
    );
    expect(result.common.artist, "artist").toBe(
    ...
    expect(result.common.picture[0].format, "picture format").toBe(
      "image/jpeg"
    );
    expect(result.common.picture[0].data.length, "picture length").toBe(
      207_439
    );
  });
});
