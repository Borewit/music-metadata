...
describe('Asynchronous observer updates', () => {
  ...
  describe('WAVE metadata before audio', () => {
    ...
    for (const sample of [
      ...
      {file: 'cest_info-id3-cset-before-data.wav', titles: ['ID3 observer title'], tagTypes: ['ID3v2.3', 'exif']},
      {file: 'cest_cset-info-id3-before-data.wav', titles: ['“Mötley” – 10 €', 'ID3 observer title'], tagTypes: ['exif', 'ID3v2.3']},
      {file: 'cest_info-id3-no-cset.wav', titles: ['ID3 observer title'], tagTypes: ['ID3v2.3', 'exif']}
    ]) {
      it(sample.file, async () => {
        const source = await openSample(sample.file);
        const titles: unknown[] = [];
        const beforeAudio: boolean[] = [];
        const metadata = await mm.parseWebStream(source.stream, {mimeType: 'audio/wav'}, {
          observer(event) {
        ...
        assert.deepEqual(beforeAudio, sample.titles.map(() => true));
